...
class Convert:
    ...
    def get_lat_long_to_meters3(self, flid, deck, X_list1, Y_list1, X_list2, Y_list2):
        distance = []
        for i in range(len(X_list1)):
            coords_1 = (X_list1[i], Y_list1[i])
            coords_2 = (X_list2, Y_list2)
            distance.insert(0,(flid[i], deck[i], X_list1[i], Y_list1[i], (geopy.distance.geodesic(coords_1, coords_2).m)))
